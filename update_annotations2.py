...
#code si le tweet ID dans la base de données est un bigint(20)
...
final_Tweet_ID_db = []
for i in range (len(connection.execute(stmt).fetchall())) : #we need to transform all elements into string to remove the L,
    first = str(connection.execute(stmt).fetchall()[i])
    final = re.sub("L,", "", first)
    final_Tweet_ID_db.append(final)
